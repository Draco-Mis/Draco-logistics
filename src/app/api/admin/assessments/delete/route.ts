...
// POST /api/admin/assessments/delete
// body: { id }
// 刪除活動；DB 上 assessment_submissions.event_id 是 ON DELETE CASCADE，所有作答紀錄會一併移除。
export async function POST(request: Request) {
  ...
  const { error } = await admin.from('assessment_events').delete().eq('id', id)
